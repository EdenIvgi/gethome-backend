...
router.get('/', (req, res, next) => {
  try {
    ...
    const result = getListings({ city, area, minPrice, maxPrice, minRooms, maxRooms, rooms, pets, parking, balcony, elevator, furnished, minFloor, maxFloor, minSizeSqm, maxSizeSqm, postedWithin, page, limit });
    res.json(result);
  } catch (err) {
    next(err);
  }
